...
static RTC_DATA_ATTR CommEspNow::RtcData s_rtc_data;
...
bool CommEspNow::init(uint32_t node_id) {
    ...
    uint32_t expected_magic = esp_rom_crc32_le(0, (uint8_t*)&m_node_id, sizeof(m_node_id));

    if (reason == ESP_RST_DEEPSLEEP && s_rtc_data.magic == expected_magic) {
        ESP_LOGI(TAG, "Woke from deep sleep, loading peers from RTC");
        for (int i = 0; i < s_rtc_data.num_peers; ++i) {
            m_peer_map[s_rtc_data.peers[i].node_id] = std::vector<uint8_t>(s_rtc_data.peers[i].mac, s_rtc_data.peers[i].mac + 6);
        }
    } else {
        ESP_LOGI(TAG, "Power-on reset or invalid RTC data, loading peers from NVS");
        s_rtc_data.magic = expected_magic;
        s_rtc_data.num_peers = 0;
        s_rtc_data.nvs_dirty_flag = false;
...
void CommEspNow::on_receive_cb(const esp_now_recv_info_t *info, const uint8_t *data, int len) {
    ...
    if (header->destination_node_id == s_instance->m_node_id || header->destination_node_id == 0xFFFFFFFF) {
...
void CommEspNow::on_send_cb(const uint8_t* mac_addr, esp_now_send_status_t status) {
     if (mac_addr == NULL) {
        ESP_LOGW(TAG, "Send cb mac error");
        return;
    }
...
void CommEspNow::learn_peer(uint32_t node_id, const uint8_t mac[6]) {
    ...
    if (it == m_peer_map.end() || it->second != mac_vec) {
        ESP_LOGI(TAG, "Learned new/updated peer: id=%lu, mac=" MACSTR, node_id, MAC2STR(mac));
        ...
        int peer_idx = -1;
        for (int i = 0; i < s_rtc_data.num_peers; ++i) {
            if (s_rtc_data.peers[i].node_id == node_id) {
                peer_idx = i;
                break;
            }
        }

        if (peer_idx == -1 && s_rtc_data.num_peers < MAX_PEERS) {
            peer_idx = s_rtc_data.num_peers++;
        }

        if (peer_idx != -1) {
            s_rtc_data.peers[peer_idx].node_id = node_id;
            memcpy(s_rtc_data.peers[peer_idx].mac, mac, 6);
            s_rtc_data.nvs_dirty_flag = true;
        } else {
...
bool CommEspNow::load_peers_from_nvs() {
    ...
    if (ret == ESP_OK && required_size > 0) {
        ...
        if (ret == ESP_OK) {
            ...
            if (required_size <= sizeof(s_rtc_data.peers)) {
                s_rtc_data.num_peers = required_size / sizeof(Peer);
                memcpy(s_rtc_data.peers, buffer.data(), required_size);
                 // Also populate the RAM map
                m_peer_map.clear();
                for (int i = 0; i < s_rtc_data.num_peers; ++i) {
                     m_peer_map[s_rtc_data.peers[i].node_id] = std::vector<uint8_t>(s_rtc_data.peers[i].mac, s_rtc_data.peers[i].mac + 6);
                }
                ESP_LOGI(TAG, "Loaded %d peers from NVS", s_rtc_data.num_peers);
            }
        }
    }
...
bool CommEspNow::save_peers_to_nvs() {
    ...
    ret = nvs_set_blob(nvs_handle, "peers", s_rtc_data.peers, s_rtc_data.num_peers * sizeof(Peer));
    if (ret == ESP_OK) {
        ret = nvs_commit(nvs_handle);
        if (ret == ESP_OK) {
            s_rtc_data.nvs_dirty_flag = false;
            ESP_LOGI(TAG, "Saved %d peers to NVS", s_rtc_data.num_peers);
        } else {
