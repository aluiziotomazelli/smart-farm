...
bool CommEspNow::send(const protocol::Frame &frame)
{
    ...
    if (protocol::encode_frame(frame, buffer, sizeof(buffer), written) !=
        protocol::CodecResult::OK) {
        return false;
    }
    ...
    if (frame.header.flags & protocol::FLAG_BROADCAST) {
        ...
        // Peer desconhecido: envia DISCOVERY_REQUEST
        ...
        disc_frame.header.node_id = frame.header.node_id;
        ...
        esp_now_send(ESPNOW_BROADCAST_ADDR, disc_buf, disc_len);
...
bool CommEspNow::receive(protocol::Frame &out)
{
    ...
    case protocol::MessageType::DISCOVERY_REQUEST:
        handle_discovery_request(out, item.src_mac);
        break;
    case protocol::MessageType::DISCOVERY_RESPONSE:
        handle_discovery_response(out, item.src_mac);
        break;
...
bool CommEspNow::resolve_peer(uint32_t node_id, uint8_t out_mac[6])
{
    return CommPeerManager::instance().resolve_peer(node_id, out_mac);
}

void CommEspNow::start_discovery(uint32_t target_node_id)
{
    m_discovery_target = target_node_id;
    m_discovery_active = true;

    // envia request imediato
    protocol::Frame disc_frame{};
    disc_frame.header.type    = protocol::MessageType::DISCOVERY_REQUEST;
    disc_frame.header.node_id = target_node_id; // 0xFFFFFFFF = todos
    disc_frame.payload_len    = 0;

    uint8_t buf[protocol::MAX_FRAME_SIZE];
    size_t  len = 0;
    protocol::encode_frame(disc_frame, buf, sizeof(buf), len);
    esp_now_send(ESPNOW_BROADCAST_ADDR, buf, len);
}

void CommEspNow::stop_discovery()
{
    m_discovery_active = false;
    m_discovery_target = 0xFFFFFFFF;
}

bool CommEspNow::load()
{
    if (!m_nvs)
        return false;
    return m_peer_mgr.load(m_nvs);
}

bool CommEspNow::save()
{
    if (!m_nvs)
        return false;
    return m_peer_mgr.save(m_nvs);
}
...
{
    protocol::Frame resp{};
    resp.header.type    = protocol::MessageType::DISCOVERY_RESPONSE;
    resp.header.node_id = target_node_id;
    resp.payload_len    = 0;

    uint8_t buf[protocol::MAX_FRAME_SIZE];
    size_t  len = 0;
    protocol::encode_frame(resp, buf, sizeof(buf), len);
    return esp_now_send(target_mac, buf, len) == ESP_OK;
}

void CommEspNow::handle_discovery_request(const protocol::Frame &frame,
                                          const uint8_t          src_mac[6])
{
    // Atualiza peer store passivamente
    CommPeerManager::instance().on_discovery_response(frame.header.node_id, src_mac);

    // Responder automaticamente apenas se discovery passivo habilitado
    if (!m_discovery_active) {
        send_discovery_response(frame.header.node_id, src_mac);
    }
}

void CommEspNow::handle_discovery_response(const protocol::Frame &frame,
                                           const uint8_t          src_mac[6])
{
    // Atualiza peer store
    CommPeerManager::instance().on_discovery_response(frame.header.node_id, src_mac);

    // Se discovery ativo, verifica se é o target
    if (m_discovery_active && (m_discovery_target == 0xFFFFFFFF ||
