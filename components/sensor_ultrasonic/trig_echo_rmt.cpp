...
TrigerEchoRmt::TrigerEchoRmt(gpio_num_t                                trig,
                             gpio_num_t                                echo,
...
bool TrigerEchoRmt::init()
{
    ESP_LOGI(TAG, "Initializing TrigerEchoRmt (trig=%d, echo=%d)", trig_pin_, echo_pin_);

    if (!TrigerEcho::init()) {
        ESP_LOGE(TAG, "Failed to initialize base TrigerEcho");
        return false;
    }
    ...
    ESP_ERROR_CHECK(rmt_new_rx_channel(&rx_cfg, &rx_channel_));
    ESP_ERROR_CHECK(rmt_enable(rx_channel_));

    return true;
}

bool TrigerEchoRmt::read_raw_distance_cm(float &out_cm, UsFailure &out_failure)
{
    ...
    if (rmt_receive(rx_channel_, rx_symbols_, sizeof(rx_symbols_), &rx_cfg) != ESP_OK) {
        out_failure = UsFailure::HW_ERROR;
        return false;
    }

    for (size_t i = 0; i < 64; i++) {
        const rmt_symbol_word_t &s = rx_symbols_[i];
