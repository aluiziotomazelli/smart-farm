#include "trig_echo_gpio.hpp"
#define LOG_LOCAL_LEVEL ESP_LOG_INFO // Must come before esp_log.h
#include "esp_log.h"
#include "esp_rom_sys.h"
#include "esp_timer.h"

static const char *TAG = "HCSR04_GPIO";

TrigerEchoGpio::TrigerEchoGpio(gpio_num_t                                trig,
                               gpio_num_t                                echo,
                               const UltrasonicSensor::UltrasonicConfig &cfg)
    : TrigerEcho{trig, cfg}
    , echo_pin_(echo)
{
}

bool TrigerEchoGpio::init()
{
    ESP_LOGI(TAG, "Initializing TrigerEchoGpio sensor (trig=%d, echo=%d)", trig_pin_,
             echo_pin_);

    if (!TrigerEcho::init()) {
        ESP_LOGE(TAG, "Failed to initialize base TrigerEcho");
        return false;
    }

    gpio_reset_pin(echo_pin_);

    gpio_config_t io_conf = {
        .pin_bit_mask = 1ULL << echo_pin_,
        .mode         = GPIO_MODE_INPUT,
        .pull_up_en   = GPIO_PULLUP_DISABLE,
        .pull_down_en = GPIO_PULLDOWN_DISABLE,
        .intr_type    = GPIO_INTR_DISABLE,
    };

    if (gpio_config(&io_conf) != ESP_OK) {
        ESP_LOGE(TAG, "Failed to configure ECHO pin");
        return false;
    }
    else {
        ESP_LOGI(TAG, "ECHO pin configured");
    }

    return true;
}

bool TrigerEchoGpio::read_raw_distance_cm(float &out_cm, UsFailure &out_failure)
{
    send_ping();

    // Echo não deveria começar alto
    if (gpio_get_level(echo_pin_) == 1) {
        out_failure = UsFailure::HW_ERROR;
        return false;
    }

    int64_t start = esp_timer_get_time();
    while (gpio_get_level(echo_pin_) == 0) {
        if (esp_timer_get_time() - start > cfg.timeout_us) {
            out_failure = UsFailure::TIMEOUT;
            return false;
        }
        esp_rom_delay_us(1);
    }

    int64_t echo_start = esp_timer_get_time();
    while (gpio_get_level(echo_pin_) == 1) {
        if (esp_timer_get_time() - echo_start > cfg.timeout_us) {
            out_failure = UsFailure::TIMEOUT;
            return false;
        }
        esp_rom_delay_us(1);
    }

    int64_t pulse = esp_timer_get_time() - echo_start;
    out_cm        = pulse * SOUND_SPEED_CM_PER_US / 2.0f;
    out_failure   = UsFailure::NONE;
    return true;
}
