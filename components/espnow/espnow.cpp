...
esp_err_t EspNow::init(const EspNowConfig &config)
{
    ...
    if (xTaskCreate(transport_worker_task, "espnow_worker", 4096, this, 5,
    ...
    while (true) {
        if (xQueueReceive(self->rx_dispatch_queue_, &packet, pdMS_TO_TICKS(1000)) == pdTRUE) {
            ...
            case MessageType::COMMAND:
                // If the message requires an ACK, save its header before passing to the
                // app
...
void EspNow::handle_pair_response(const RxPacket &packet)
{
    ...
    if (response->status == PairStatus::ACCEPTED) {
        ...
        add_peer(response->header.sender_node_id, packet.src_mac, response->wifi_channel,
                 response->header.sender_type);
...
void EspNow::tx_manager_task(void *arg)
{
    ...
    while (true) {
        ...
        case TxState::IDLE:
        {
            ...
            // Wait for any notification. Clear all bits on exit to avoid busy loop.
            if (xTaskNotifyWait(0, ULONG_MAX, &notifications, pdMS_TO_TICKS(1000)) == pdTRUE) {
        ...
        {
            // Clear all bits on exit to avoid busy loop.
            if (xTaskNotifyWait(0, ULONG_MAX, &notifications, pdMS_TO_TICKS(1000)) == pdPASS) {
                ...
                // Even while waiting for ACK, we must process other async notifications
                if (notifications & NOTIFY_HEARTBEAT) {
                    self->send_heartbeat();
                }
                if (notifications & NOTIFY_PAIRING) {
