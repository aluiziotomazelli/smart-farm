...
TEST_CASE("GpioValidator: Chip-specific Flash pins (Negative)", "[gpio][validator][negative]")
{
    ...
    if (chip_info.model == CHIP_ESP32) {
        ...
        // Pins 6-11 are flash
        TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, GpioValidator::validate(GPIO_NUM_6, GpioValidator::Mode::OUTPUT));
        TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, GpioValidator::validate(GPIO_NUM_11, GpioValidator::Mode::OUTPUT));
        // Pins 16-17 are often PSRAM/Flash on some modules
        TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, GpioValidator::validate(GPIO_NUM_16, GpioValidator::Mode::OUTPUT));
    }
    else if (chip_info.model == CHIP_ESP32S3) {
        ESP_LOGI(TAG, "Running ESP32-S3-specific flash pin tests");
        // Pins 26-32 are flash
        TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, GpioValidator::validate(GPIO_NUM_26, GpioValidator::Mode::OUTPUT));
        TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, GpioValidator::validate(GPIO_NUM_32, GpioValidator::Mode::OUTPUT));
    }
    else if (chip_info.model == CHIP_ESP32C3) {
        ESP_LOGI(TAG, "Running ESP32-C3-specific flash pin tests");
        // Pins 12-17 are flash
        TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, GpioValidator::validate(GPIO_NUM_12, GpioValidator::Mode::OUTPUT));
        TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, GpioValidator::validate(GPIO_NUM_17, GpioValidator::Mode::OUTPUT));
    }
}
...
TEST_CASE("GpioValidator: Input-only pins (Negative Output)", "[gpio][validator][negative]")
{
    ...
    if (chip_info.model == CHIP_ESP32) {
        ...
        TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, GpioValidator::validate(GPIO_NUM_34, GpioValidator::Mode::OUTPUT));
        TEST_ASSERT_EQUAL(ESP_OK, GpioValidator::validate(GPIO_NUM_34, GpioValidator::Mode::INPUT));
    }
}
...
TEST_CASE("GpioValidator: Warnings for dedicated pins", "[gpio][validator]")
{
    ...
    if (chip_info.model == CHIP_ESP32) {
        // Strapping / JTAG
        TEST_ASSERT_EQUAL(ESP_OK, GpioValidator::validate(GPIO_NUM_0, GpioValidator::Mode::OUTPUT));
        TEST_ASSERT_EQUAL(ESP_OK, GpioValidator::validate(GPIO_NUM_12, GpioValidator::Mode::OUTPUT));
        // UART0
        TEST_ASSERT_EQUAL(ESP_OK, GpioValidator::validate(GPIO_NUM_1, GpioValidator::Mode::OUTPUT));
    }
    else if (chip_info.model == CHIP_ESP32S3) {
        // USB-JTAG
        TEST_ASSERT_EQUAL(ESP_OK, GpioValidator::validate(GPIO_NUM_19, GpioValidator::Mode::OUTPUT));
        // Strapping
        TEST_ASSERT_EQUAL(ESP_OK, GpioValidator::validate(GPIO_NUM_0, GpioValidator::Mode::OUTPUT));
    }
}
